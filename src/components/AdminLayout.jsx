    ...
    >
      ...
      <aside className="relative z-20 overflow-visible bg-white/10 backdrop-blur-xl border-r border-white/10 p-4 flex flex-col text-white">
        ...
        <nav className="space-y-3">
          <NavItem to="/admin/dashboard" label="Dashboard" icon="🛡️" collapsed={!expanded} />
          <NavItem to="/admin/users" label="User Management" icon="👥" collapsed={!expanded} />
          <NavItem to="/admin/attendance" label="Attendance" icon="📋" collapsed={!expanded} />
          <NavItem to="/admin/jobs" label="Jobs" icon="💼" collapsed={!expanded} />
          <NavItem to="/admin/interviews" label="Interviews" icon="🗓️" collapsed={!expanded} />
          <NavItem to="/admin/documents" label="Documents" icon="📄" collapsed={!expanded} />
          <NavItem to="/admin/reports" label="Reports" icon="📊" collapsed={!expanded} />
          <NavItem to="/admin/settings" label="Settings" icon="⚙️" collapsed={!expanded} />
        </nav>
        ...
        <header className="backdrop-blur-xl bg-white/10 border-b border-white/10 text-white">
          <div className="px-6 py-3 flex items-center justify-between">
            <div className="text-sm flex items-center gap-3">
              <span>Admin</span>
              <span className="px-2 py-0.5 rounded-full bg-white/15 border border-white/20 text-xs">Area: Admin</span>
            </div>
