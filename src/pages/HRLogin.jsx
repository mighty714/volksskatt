...
export default function HRLogin() {
  const [email, setEmail] = useState('HR')
  ...
  const submit = async (e) => {
    e.preventDefault()
    const ok = await login(email, password, 'hr')
    if (ok) {
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="flex items-center justify-center md:justify-start md:-translate-x-4">
            <div className="w-full flex flex-col items-center pl-0">
              <div className="text-center">
                <div className="text-white text-4xl md:text-5xl font-semibold tracking-tight">Volksskatt</div>
                <div className="text-white/90 text-2xl md:text-3xl font-medium -mt-1 tracking-tight">Infotech</div>
              </div>
              <div className="mt-8">
          ...
          <form onSubmit={submit} className="bg-transparent rounded-3xl p-6 md:p-8 shadow-2xl border border-transparent w-full max-w-md min-h-[520px] flex flex-col">
            <div className="mb-6 text-center">
              <div className="text-white text-3xl md:text-4xl font-extrabold tracking-tight drop-shadow">HR Login</div>
            ...
            <div className="relative mb-4">
              <input className="w-full pl-9 pr-3 py-3 bg-transparent border-0 border-b-2 border-orange-400 placeholder-orange-400 focus:outline-none focus:border-orange-500" value={email} onChange={(e) => setEmail(e.target.value)} type="text" placeholder="hr@company.com" required />
            ...
            <div className="relative mb-4">
              <input className="w-full pl-9 pr-3 py-3 bg-transparent border-0 border-b-2 border-orange-400 placeholder-orange-400 focus:outline-none focus:border-orange-500" value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="••••••••••" required />
              <span className="absolute left-1 top-1/2 -translate-y-1/2 text-orange-500">🔒</span>
            </div>
        ...
        <div className="fixed inset-0 z-30 grid place-items-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white/90 backdrop-blur-md shadow-2xl p-6">
            ...
            <form onSubmit={submitForgot}>
              <label className="block text-xs font-semibold tracking-wide text-orange-600 mb-1">EMAIL</label>
              <input className="w-full px-3 py-2 rounded-lg border border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-white/80 text-slate-900 placeholder-slate-500" type="email" placeholder="you@example.com" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} required />
              {forgotStatus.message && (<div className="mt-3 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">{forgotStatus.message}</div>)}
              <div className="mt-4 flex gap-3">
                ...
                <button type="submit" disabled={forgotStatus.loading} className="px-4 py-2 rounded-lg bg-[#f57c00] text-white hover:bg-[#e96f00] disabled:opacity-60 disabled:cursor-not-allowed">{forgotStatus.loading ? 'Sending…' : 'Send reset link'}</button>
              </div>
            </form>
          </div>
        </div>
