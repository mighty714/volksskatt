...
export default function EmployeeLogin() {
  ...
  const submit = async (e) => {
    e.preventDefault()
    const ok = await login(email, password, 'employee')
    if (ok) {
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <form onSubmit={submit} className="bg-transparent rounded-3xl p-6 md:p-8 shadow-2xl border border-transparent w-full max-w-md min-h-[520px] flex flex-col">
            <div className="mb-6 text-center">
              <div className="text-white text-3xl md:text-4xl font-extrabold tracking-tight drop-shadow">Employee Login</div>
            ...
            <div className="relative mb-4">
              <input className="w-full pl-9 pr-3 py-3 bg-transparent border-0 border-b-2 border-orange-400 placeholder-orange-400 focus:outline-none focus:border-orange-500" value={email} onChange={(e) => setEmail(e.target.value)} type="text" placeholder="user@company.com" required />
