...
export default function HrClock() {
  const user = getUser()
  const defaultEmp = useMemo(() => ({
    empId: user?.email || 'hr-user',
    empName: user?.fullName || 'HR User',
    empRole: user?.role || 'hr'
  }), [user])

  const { rows, status, todayEntry, actions } = useClock({
    storageKey: 'attendance_hr',
    defaultEmp,
  })
  ...
  return (
    <section className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">HR Clock</h2>
        ...
        <table className="min-w-full text-sm border-collapse">
          ...
          <tbody>
            {rows.length > 0 ? (
              ...
              <tr>
                <td colSpan={9} className="p-4 text-center text-gray-500">No attendance records yet.</td>
              </tr>
            )}
          </tbody>
        </table>
